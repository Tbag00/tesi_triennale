...
L'obiettivo dello studio è quello di applicare il \textit{framework} su un \textit{dataset} di \textbf{annunci di lavoro}, per poter capire quali sono i \textbf{requisiti}, \textbf{mansioni} e \textbf{competenze} più richieste nel mercato del lavoro in ambito di \textbf{Data Analytics}.
Abbiamo deciso di struttuare lo studio come segue:
\begin{enumerate}
    \item \textit{Analisi preliminare e acquisizione dei dati}: Il capitolo iniziale riprende uno studio che ha già affrontato il topic modeling nello stesso dominio e, a partire da esso, definisce il processo di raccolta del \textit{dataset}, illustrandone le peculiarità.
    \item \textit{BERTopic}: Il capitolo seguente illustra la struttura del \textit{framework}, ne mette in evidenza gli elementi distintivi e approfondisce i moduli che lo compongono.
    \item \textit{Implementazione}: Qui vengono motivate le scelte di parametrizzazione adottate per BERTopic in relazione alla natura del \textit{dataset} e agli obiettivi della ricerca.
    \item \textit{Pre-processing}: Questo capitolo dettaglia il trattamento del \textit{dataset} grezzo necessario a renderlo compatibile con il \textit{framework} e a estrarre gli argomenti pertinenti allo studio.
    \item \textit{Risultati}: La sezione esamina gli esiti ottenuti, ne discute le implicazioni e mette in risalto gli aspetti più significativi.
    \item \textit{Conclusioni}: L'ultimo capitolo sintetizza i passaggi decisivi dell'indagine, evidenziando possibili miglioramenti, criticità e sviluppi futuri.
\end{enumerate}